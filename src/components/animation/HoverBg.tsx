...
/**
 * HoverBg Component
 * Creates a mouse-following background grid SVG spotlight effect.
 * Uses requestAnimationFrame interpolation for a buttery-smooth mask reveal.
 */
export default function HoverBg({ className = "" }: HoverBgProps) {
  const glowRef = useRef<HTMLDivElement>(null);

  // Track visibility to fade out when the mouse leaves the window
  const [isVisible, setIsVisible] = useState(false);

  const positionRef = useRef({ targetX: 0, targetY: 0, currentX: 0, currentY: 0, initialized: false });
  ...
  const hasOpacityClass = className
    .split(" ")
    .some((c) => c.startsWith("opacity-") || c.startsWith("dark:opacity-") || c.includes(":opacity-"));

  return (
    <div
      ref={glowRef}
      className={`absolute inset-0 w-full h-full pointer-events-none -z-10 transition-opacity duration-500 ${hasOpacityClass ? "" : "opacity-60 dark:opacity-40"
        } ${className}`}
      style={{
        opacity: isVisible ? undefined : 0,
        // Default mask in case mouse hasn't moved yet
        WebkitMaskImage: "radial-gradient(circle 180px at 50% 50%, black, transparent)",
        maskImage: "radial-gradient(circle 180px at 50% 50%, black, transparent)",
      }}
    >
      {/* The SVG grid pattern repeated across the screen and revealed by the mask */}
      <svg className="absolute inset-0 w-full h-full text-primary/50 dark:text-primary/35 pointer-events-none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="glow-grid-pattern" patternUnits="userSpaceOnUse" width="574" height="574">
            <SparkleGrid />
          </pattern>
        </defs>
