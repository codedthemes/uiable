...
import { loadCatalog, getCatalogItem, loadRegistryItem, type CatalogItem } from "../catalog.js";
...
const notFound = (name: string) => ({
  content: [{ type: "text" as const, text: `No uiable item named "${name}". Use search_blocks or list_components to find valid names.` }],
  isError: true,
});

// Compact projection for list/search results (omits keywords/urls noise).
const summary = (i: CatalogItem) => ({
  name: i.name,
  kind: i.kind,
  title: i.title,
...
function score(item: CatalogItem, terms: string[]): number {
  const hay = {
  ...
  for (const t of terms) {
    if (hay.cats.split(" ").includes(t)) s += 5;
    if (hay.title.includes(t)) s += 4;
    if (hay.kw.includes(t)) s += 3;
    if (hay.name.includes(t)) s += 2;
...
export function registerDiscoveryTools(server: McpServer): void {
  server.registerTool(
    "list_blocks",
    {
      ...
      description:
        "List page-level uiable blocks (hero, pricing, feature, footer, etc.). Optionally filter by category and paginate.",
      inputSchema: {
    ...
    async ({ category, limit = 50, offset = 0 }) => {
      ...
      if (category) blocks = blocks.filter((i) => i.categories.includes(category));
  ...
  server.registerTool(
    "list_components",
    {
      ...
      description:
        "List primitive uiable components (button, input, card, accordion, etc.). Optionally filter by category and paginate.",
      inputSchema: {
    ...
    async ({ category, limit = 50, offset = 0 }) => {
      ...
      if (category) comps = comps.filter((i) => i.categories.includes(category));
  ...
  server.registerTool(
    "search_blocks",
    {
      ...
      description:
        "Keyword/intent search across all uiable blocks and components. Returns ranked matches with previews and install commands.",
      inputSchema: {
  ...
  server.registerTool(
    "get_block",
    {
      ...
      description:
        "Get full details for one uiable block/component by name: metadata, source file(s) with contents, preview URL, and install command.",
      inputSchema: {
  ...
  server.registerTool(
    "get_add_command",
    {
      ...
      description:
        "Return the `npx shadcn add` command(s) to install one or more uiable items. Validates each name against the catalog.",
      inputSchema: {
