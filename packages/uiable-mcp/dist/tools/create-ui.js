...
function pickForCategory(blocks, category, terms) {
...
const stripExt = (p) => p.replace(/\.[tj]sx?$/, "");
...
const importPath = (target) => `@/${stripExt(target)}`;
function buildScaffold(picks) {
    const imports = picks.map((p) => `import ${p.component} from "${p.importPath}";`).join("\n");
    const body = picks.map((p) => `      {/* ${p.section} */}\n      <${p.component} />`).join("\n");
    return `${imports}\n\nexport default function Page() {\n  return (\n    <main className="flex min-h-screen flex-col">\n${body}\n    </main>\n  );\n}\n`;
}
export function registerCreateUiTool(server) {
    server.registerTool("create_ui", {
        title: "Compose a page from uiable blocks",
        description: "Generate a page composition from uiable blocks for a natural-language intent. Returns the ordered blocks chosen, an assembled page.tsx scaffold wiring them together, and the shadcn add commands to install them. The host AI should run the add commands and write the scaffold.",
        inputSchema: {
        ...
        return json({
            ...
            instructions: "Run addCommand to install the blocks, write scaffold.content to scaffold.path, then adjust copy/props to match the intent. Sections in missingSections had no matching uiable block — build or omit them.",
        });
    });
}
